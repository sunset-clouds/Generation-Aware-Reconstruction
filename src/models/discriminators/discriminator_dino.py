...
DINO_DEIT_SMALL16_PATH = os.environ.get(
    "DINO_CKPT_PATH",
    os.path.join("assets", "checkpoints", "dino_deitsmall16_pretrain.pth"),
)


def _load_dino_state_dict(dino_ckpt_path):
    """Load DINO state dict from env DINO_CKPT_PATH or a local file."""
    local_path = os.environ.get('DINO_CKPT_PATH', '').strip()
    if local_path and os.path.isfile(local_path):
        return torch.load(local_path, map_location='cpu', weights_only=False)
    if os.path.isfile(dino_ckpt_path):
        return torch.load(dino_ckpt_path, map_location='cpu', weights_only=False)
    raise FileNotFoundError(
        "DINO checkpoint not found. Set DINO_CKPT_PATH or pass a local "
        f"dino_ckpt_path. Missing path: {dino_ckpt_path}"
    )


class DinoDisc(nn.Module):
    def __init__(self, dino_ckpt_path=DINO_DEIT_SMALL16_PATH,
                 device='cuda', ks=9, depth=12, key_depths=(2, 5, 8, 11), norm_type='sbn',
                 using_spec_norm=True, norm_eps=1e-6):
        super().__init__()
        # load state (prefer env DINO_CKPT_PATH or local file to avoid URL returning HTML)
        state = _load_dino_state_dict(dino_ckpt_path)
...
if __name__ == '__main__':
    ...
    # Optional local DINO checkpoint path can be provided via DINO_CKPT_PATH.
    ckpt = DINO_DEIT_SMALL16_PATH
    DinoDisc.forward
