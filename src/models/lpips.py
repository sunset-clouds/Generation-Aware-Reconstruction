...
We modified the LPIPS.forward with another scaling/shifting the input tensor
from [0, 1] to [-1, 1]. Because our code-base defaults to normalizing image
to [0, 1] while LPIPS expects an input normalized to [-1, 1].
"""
...
URL_MAP = {
    "vgg_lpips": os.environ.get(
        "LPIPS_VGG_CKPT",
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "cache", "vgg.pth"),
    )
}
...
def get_ckpt_path(name, root, check=False):
    ...
    if not os.path.exists(path) or (check and not md5_hash(path) == MD5_MAP[name]):
        source_path = URL_MAP[name]
        if not os.path.exists(source_path):
            raise FileNotFoundError(
                f"LPIPS checkpoint not found. Set LPIPS_VGG_CKPT or place it at {path}."
            )
