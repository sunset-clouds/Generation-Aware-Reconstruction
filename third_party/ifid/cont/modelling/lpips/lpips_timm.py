            ...
            if self.intermediate_loss:
                ...
                if dino_variants == "depth12_no_train":
                    ...
                    self.lins = nn.ModuleList(
                        [NetLinLayer(self.model.embed_dim, use_dropout=use_dropout) for _ in self.key_depths]
                    )
            self.image_size = 256
    ...
    def forward(self, input, target):
        ...
        if self.intermediate_loss:
            if self.key_depths is not None:
                for depth in self.key_depths:
                    diff = (normalize_tensor(outs0_feats[depth]) - normalize_tensor(outs1_feats[depth])) ** 2
                    diffs[diff_cnt] = diff
                    diff_cnt += 1
            else:
        ...
        if self.intermediate_loss:
            if self.key_depths is not None:
                for i, _ in enumerate(self.key_depths):
