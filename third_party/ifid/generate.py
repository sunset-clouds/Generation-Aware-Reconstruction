...
def denormalize_latents(latents, latents_scale, latents_bias):
    """Local copy to avoid importing train.py and its training-only deps."""
    return latents / latents_scale + latents_bias


def save_tensor_image(x, path):
    """Save a tensor image in [-1, 1] or [0, 1] to disk."""
    x = x.detach().cpu()
    if x.dim() == 4:
        x = x[0]
    if x.min().item() < 0:
...
VAE_CONFIG_HINTS = {
    "f8d4": "./configs/SDVAE.yaml",
    "sdvae": "./configs/SDVAE.yaml",
    "eqvae": "./configs/EQVAE.yaml",
    "eq-vae": "./configs/EQVAE.yaml",
    "invae": "./configs/INVAE.yaml",
    "flux": "./configs/FLUXVAE.yaml",
    "fluxvae": "./configs/FLUXVAE.yaml",
    "qw": "./configs/QWVAE.yaml",
    "qwvae": "./configs/QWVAE.yaml",
    "sd3": "./configs/SD3VAE.yaml",
    "sd3vae": "./configs/SD3VAE.yaml",
    "vavae64": "./configs/VAVAE64.yaml",
    "vavae": "./configs/VAVAE.yaml",
    "softvq": "./configs/SOFTVQ.yaml",
    "maetok": "./configs/MAETOK.yaml",
    "detok": "./configs/DETOK.yaml",
    "dmvae": "./configs/DMVAE.yaml",
    "repae": "./configs/REPAEVAE.yaml",
    "repaevae": "./configs/REPAEVAE.yaml",
    "rae": "./configs/RAE.yaml",
}


def resolve_vae_config_path(config, exp_path):
    """Recover vae_config when checkpoint args.json is incomplete."""
    vae_config_value = getattr(config, "vae_config", None)
    if isinstance(vae_config_value, str) and vae_config_value.strip():
        return vae_config_value.strip()

    exp_name = os.path.basename(exp_path).lower()
    vae_tag = str(getattr(config, "vae", "")).strip().lower()
    search_texts = [exp_name, vae_tag]
    for hint, rel_path in VAE_CONFIG_HINTS.items():
        if any(hint in text for text in search_texts if text):
            return rel_path

    raise ValueError(
        "Could not infer vae_config from checkpoint args.json. "
        f"exp_path={exp_path!r}, vae={vae_tag!r}"
    )
...
def main(args):
    ...
    vae_config_path = resolve_vae_config_path(config, args.exp_path)
    if rank == 0:
        print(f"Using vae_config: {vae_config_path}")
    ...
    resumed_existing = 0
    for _ in pbar:
        batch_indices = [i * dist.get_world_size() + rank + total for i in range(n)]
        if all(
            os.path.exists(f"{sample_folder_dir}/{index:06d}.png")
            and os.path.getsize(f"{sample_folder_dir}/{index:06d}.png") > 0
            for index in batch_indices
        ):
            resumed_existing += len(batch_indices)
    ...
    if resumed_existing and rank == 0:
        print(f"Resumed existing samples on rank 0: {resumed_existing}")
    ...
    if rank == 0:
        ...
        print(f"Keeping samples at {sample_folder_dir}")
        # shutil.rmtree(sample_folder_dir)
    dist.barrier()
